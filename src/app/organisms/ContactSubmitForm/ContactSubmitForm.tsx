...
const schema = Yup.object().shape({
    ...
    email: Yup.string().required("メールを空にすることはできません。").email("電子メールの形式が無効です。"),
    confirmedEmail: Yup.string().required("確認メールを空にすることはできません。").email("メールの形式が無効であることを確認してください。").equals([Yup.ref("email")], 'メールアドレスが合わない！'),
...
const ContactSubmitForm: React.FC<IProps> = () => {
    ...
    return <div >
        {stage === STAGES.FORM && <ContactForm next={() => setStage(STAGES.REVIEW)} form={form} />}
        {stage === STAGES.REVIEW && <ContactFormReview onSubmit={handleSubmit} submitted={submitted} back={() => setStage(STAGES.FORM)} data={form.getValues() as any} />}
    </div>
}
